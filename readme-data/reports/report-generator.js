...
const createDistributionChart = async (distributionChart) => {
    const data = {
...
const createPerformanceChart = async (performanceChartData) => {
...
const analyseSolutionsDistribution = async (analysedReportFiles) => {
    ...
    const table = `| Optimal solution length | Distribution | ${algorithmsConstantProperties.map(item => `${item.name} times avg (std. dev.) `).join(' | ')} | \n` +
...
const aggregateResults = (analysedReportFiles) => {
    const table = `| Algorithm | Time average - (std. dev.) | Average times worse than the best | Visited nodes (std. dev.) | Optimal solution length ratio average | Time complexity | Spacial complexity |\n` +
        `| ----- | ----- | ----- | ----- | ----- | ----- | ----- |\n` +
