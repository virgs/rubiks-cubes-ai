...
const analyseSolutionsDistribution = () => {
    ...
    const table =
        `| Optimal solution size | Appearances | Distribution | ${algorithmsConstantProperties.map(name => `${name} avg times (std. dev.) `).join(' | ')} | \n` +
...
const aggregateResults = () => {
    const table =
        `| Algorithm | Time average - (std. dev.) | Average time worse than the best | Visited nodes (std. dev.) | Optimal solution length ratio average | Time complexity | Spacial complexity |\n` +
        `| ----- | ----- | ----- | ----- | ----- | ----- | ----- |\n` +
